...
type DeliveryVisualizerProps = {
	delivery?: IDelivery | null;
};

function DeliveryVisualizer({ delivery }: DeliveryVisualizerProps) {
	...
	return (
		<div className="flex flex-col bg-white rounded-3xl p-5 basis-[400px] h-full gap-5">
			<ConfirmationModal
				...
				message={`Deseja mesmo cancelar a entrega #${delivery!.id
					.split('-')[0]
			...
			<header className="flex flex-row justify-between gap-5">
				...
				<div className="text-right">
					...
					<p className="text-lg font-semibold">{delivery!.user.name}</p>
				</div>
			</header>
			...
			<footer className="flex flex-col gap-5">
				<div className="text-xl flex flex-row justify-between">
					...
					<h1 className="font-semibold">{BRL(delivery?.totalPrice!)}</h1>
				</div>
...
export default DeliveryVisualizer;
